use crate::memory::{Page, PAGE_SIZE, PAGE_SIZE_U32};
use qor_core::{interfaces::mmio::MMIOInterface, memory::allocators::page::bitmap::PageBox};

use super::{
    bits, raw,
    structures::{DeviceID, Queue, VirtIOError},
};
...
impl VirtIOWrapper {
    ...
    pub fn set_status_bits(&self, mask: u32) -> Result<(), VirtIOError> {
        // Safety: The only safe way to construct a `VirtIOWrapper` is by providing a proper base address.
        unsafe { raw::atomic_status_register(&self.mmio_layer) }
    ...
    pub fn add_queue(
        &self,
        index: u32,
        allocator: impl Fn() -> PageBox<'static, Page, Queue>,
    ) -> Result<PageBox<'static, Page, Queue>, VirtIOError> {
        let boxed_queue = allocator();

        let ptr = boxed_queue.as_ptr();
        let pfn = (ptr as usize / PAGE_SIZE)
            .try_into()
            .expect("Pointer too wide");
